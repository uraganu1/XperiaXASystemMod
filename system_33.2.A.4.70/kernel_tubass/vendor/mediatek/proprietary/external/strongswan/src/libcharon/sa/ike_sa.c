	...
	private_ike_sa_t *this, bool address)
{
	...
	if (get_cust_setting_bool(ADDR_CHANGE_N_REAUTH)) {
		...
		/* since our previous path is not valid anymore,
...
#ifdef ANDROID
	property_get("persist.net.wo.keep_timer", sysvalue, "");
	if (strlen(sysvalue)) {
		DBG1(DBG_IKE, "NAT keepalive timer change to %s", sysvalue);
		this->keepalive_interval = atoi(sysvalue);
	}
#endif
