...
#define EEPROM_DRVNAME "CAT24C16_CAL_DRV"
...
//	EEPROMDB("selective_read_byte\n");
	...
	if(iReadRegI2C(&offset, 1, (u8*)data, 1, i2c_id+(page<<1))<0) {
		EEPROMDB("fail selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr, *data,page,offset);
		return false;
	}
	//EEPROMDB("selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr, *data,page,offset);
    return true;
}
...
int selective_read_region(u32 addr, BYTE* data,u16 i2c_id,u32 size)
{
    ...
    while(size_to_read>0) {
		if(selective_read_byte(addr,(u8*)buff,CAT24C16_DEVICE_ID)){
			addr+=1;
			buff+=1;
			size_to_read-=1;
    		ret+=1;
	...
	EEPROMDB("selective_read_region addr =%x size %d data read = %d\n", addr,size, ret);
    return ret;
}

int iReadDataFromCAT24c16(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
...
static long CAT24c16_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
    EEPROMDB("[CAT24C16] COMPAT_CAM_CALIOC_G_READ\n");
      EEPROMDB("[CAT24C16] CAT24c16_Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
    ...
    case COMPAT_CAM_CALIOC_G_READ:
    {
        ...
        if(err != 0)
            EEPROMDB("[CAT24C16] compat_put_acdk_sensor_getinfo_struct failed\n");
        return ret;
    }
    default:
        return -ENOIOCTLCMD;
...
*/
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (NULL == pBuff) {
            EEPROMDB("[CAT24C16] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff , (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
                EEPROMDB("[CAT24C16] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
    ...
    EEPROMDB("[CAT24C16] init Working buffer address 0x%p  command is 0x%8x\n", pWorkingBuff, (u32)a_u4Command);
	...
	case CAM_CALIOC_G_READ:
        ...
        EEPROMDB("[CAT24C16] After read Working buffer data  0x%4x\n", *pWorkingBuff);
#else
		if (ptempbuf->u4Offset == 0x0024C32a) {
			*(u32 *)pWorkingBuff = 0x0124C32a;
			...
			while (0 < readTryagain) {
                //i4RetValue =  iReadDataFromCAT24c16((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
				i4RetValue = selective_read_region(ptempbuf->u4Offset, pWorkingBuff, CAT24C16_DEVICE_ID,ptempbuf->u4Length);
                EEPROMDB("[CAT24C16] error (%d) Read retry (%d)\n", i4RetValue, readTryagain);
				if (i4RetValue == 0)
					readTryagain--;
				else
					readTryagain = 0;
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
        ...
        EEPROMDB("[CAT24C16] to user length %d\n", ptempbuf->u4Length);
        EEPROMDB("[CAT24C16] to user  Working buffer address 0x%p\n", pWorkingBuff);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
            EEPROMDB("[CAT24C16] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
static inline int RegisterEEPROMCharDrv(void)
{
	...
	if (NULL == g_pEEPROM_CharDrv) {
        ...
        EEPROMDB("[CAT24C16] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pEEPROM_CharDrv, g_EEPROMdevno, 1)) {
        EEPROMDB("[CAT24C16] Attatch file operation failed\n");
    ...
    EEPROM_class = class_create(THIS_MODULE, "CAT24c16_CAM_CALdrv");
...
static unsigned short force[] = {EEPROM_I2C_GROUP_ID, CAT24C16_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};
#else
/* static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, CAT24C16_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END}; */
#endif
...
{
	int i4RetValue = 0;
        EEPROMDB("CAT24C16 EEPROM_i2c_probe\n");
    EEPROMDB("[CAT24C16] Attach I2C\n");
	...
	if (i4RetValue) {
        EEPROMDB("[CAT24C16] register char device failed!\n");
		return i4RetValue;
	}


    EEPROMDB("[CAT24C16] Attached!!\n");
	return 0;
}
...
{
        EEPROMDB("CAT24C16 EEPROM_probe\n");
...
static int __init EEPROM_i2C_init(void)
{
	i2c_register_board_info(EEPROM_I2C_BUSNUM, &kd_eeprom_dev, 1);
        EEPROMDB("CAT24C16 EEPROM_i2C_init\n");

	if (platform_device_register(&g_stEEPROM_Device)) {
        EEPROMDB("failed to register CAT24C16 driver, 2nd time\n");
		return -ENODEV;
	}
        EEPROMDB("CAT24C16 EEPROM_i2C_init step1\n");
	if (platform_driver_register(&g_stEEPROM_Driver)) {
        EEPROMDB("failed to register CAT24C16 driver\n");
		return -ENODEV;
	}
        EEPROMDB("CAT24C16 EEPROM_i2C_init step2\n");
