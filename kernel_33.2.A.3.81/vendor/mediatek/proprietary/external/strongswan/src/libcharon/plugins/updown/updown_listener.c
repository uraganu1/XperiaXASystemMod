...
static void notify_wod_pcscf(const char* apn, conn_info_prop* prop)
{
	char tmp[TX_BUF_LEN] = "";
	char buf[TX_BUF_LEN] = "";
	int n_pcscf6 = 0;
	...
	for (n_pcscf6 = 0; prop->pcscf6[n_pcscf6][0] && n_pcscf6 < MAX_PCSCF_NUM;
			++n_pcscf6) {
		snprintf(tmp, TX_BUF_LEN, "%s,%s", tmp, prop->pcscf6[n_pcscf6]);
	}
	for (n_pcscf4 = 0; prop->pcscf[n_pcscf4][0] && n_pcscf4 < MAX_PCSCF_NUM;
			++n_pcscf4) {
		snprintf(tmp, TX_BUF_LEN, "%s,%s", tmp, prop->pcscf[n_pcscf4]);
	}
	snprintf(buf, TX_BUF_LEN, "ipsecpcscf=%s,%d,%d%s", apn + 4, n_pcscf6, n_pcscf4, tmp);
	DBG1(DBG_CHD, "go to send: %s", buf);
	wod_tcp_tx(buf, strlen(buf));
}

static void notify_wod_dns(const char* apn, conn_info_prop* prop)
{
	char tmp[TX_BUF_LEN] = "";
	char buf[TX_BUF_LEN] = "";
	int n_dns6 = 0;
	...
	for (n_dns6 = 0; prop->dns6[n_dns6][0] && n_dns6 < MAX_DNS_NUM; ++n_dns6) {
		snprintf(tmp, TX_BUF_LEN, "%s,%s", tmp, prop->dns6[n_dns6]);
	}
	for (n_dns4 = 0; prop->dns[n_dns4][0] && n_dns4 < MAX_DNS_NUM; ++n_dns4) {
		snprintf(tmp, TX_BUF_LEN, "%s,%s", tmp, prop->dns[n_dns4]);
	}
	snprintf(buf, TX_BUF_LEN, "ipsecdns=%s,%d,%d%s", apn + 4, n_dns6, n_dns4, tmp);
	DBG1(DBG_CHD, "go to send: %s", buf);
	wod_tcp_tx(buf, strlen(buf));
}
...
//	if (!up && !ike_sa->has_condition(ike_sa, COND_REAUTHENTICATING)) {
	if (!up) {
        ...
        if (this->prop.pcscf6[0][0] || this->prop.pcscf[0][0])
    		notify_wod_pcscf(config->get_name(config), &this->prop);
        if (this->prop.dns6[0][0] || this->prop.dns[0][0])
		    notify_wod_dns(config->get_name(config), &this->prop);
