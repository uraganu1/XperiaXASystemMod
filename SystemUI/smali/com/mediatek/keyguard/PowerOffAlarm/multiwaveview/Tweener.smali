.class Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
.super Ljava/lang/Object;
.source "Tweener.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener$1;
    }
.end annotation


# static fields
.field private static mCleanupListener:Landroid/animation/Animator$AnimatorListener;

.field private static sTweens:Ljava/util/HashMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/HashMap",
            "<",
            "Ljava/lang/Object;",
            "Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;",
            ">;"
        }
    .end annotation
.end field


# instance fields
.field animator:Landroid/animation/ObjectAnimator;


# direct methods
.method static synthetic -wrap0(Landroid/animation/Animator;)V
    .locals 0
    .param p0, "animator"    # Landroid/animation/Animator;

    .prologue
    invoke-static {p0}, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->remove(Landroid/animation/Animator;)V

    return-void
.end method

.method static constructor <clinit>()V
    .locals 1

    .prologue
    .line 38
    new-instance v0, Ljava/util/HashMap;

    invoke-direct {v0}, Ljava/util/HashMap;-><init>()V

    sput-object v0, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    .line 141
    new-instance v0, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener$1;

    invoke-direct {v0}, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener$1;-><init>()V

    sput-object v0, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->mCleanupListener:Landroid/animation/Animator$AnimatorListener;

    .line 33
    return-void
.end method

.method public constructor <init>(Landroid/animation/ObjectAnimator;)V
    .locals 0
    .param p1, "anim"    # Landroid/animation/ObjectAnimator;

    .prologue
    .line 40
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 41
    iput-object p1, p0, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->animator:Landroid/animation/ObjectAnimator;

    .line 40
    return-void
.end method

.method private static remove(Landroid/animation/Animator;)V
    .locals 3
    .param p0, "animator"    # Landroid/animation/Animator;

    .prologue
    .line 45
    sget-object v2, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    invoke-virtual {v2}, Ljava/util/HashMap;->entrySet()Ljava/util/Set;

    move-result-object v2

    invoke-interface {v2}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v1

    .line 46
    .local v1, "iter":Ljava/util/Iterator;, "Ljava/util/Iterator<Ljava/util/Map$Entry<Ljava/lang/Object;Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;>;>;"
    :cond_0
    invoke-interface {v1}, Ljava/util/Iterator;->hasNext()Z

    move-result v2

    if-eqz v2, :cond_1

    .line 47
    invoke-interface {v1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/util/Map$Entry;

    .line 48
    .local v0, "entry":Ljava/util/Map$Entry;, "Ljava/util/Map$Entry<Ljava/lang/Object;Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;>;"
    invoke-interface {v0}, Ljava/util/Map$Entry;->getValue()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;

    iget-object v2, v2, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->animator:Landroid/animation/ObjectAnimator;

    if-ne v2, p0, :cond_0

    .line 51
    invoke-interface {v1}, Ljava/util/Iterator;->remove()V

    .line 44
    .end local v0    # "entry":Ljava/util/Map$Entry;, "Ljava/util/Map$Entry<Ljava/lang/Object;Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;>;"
    :cond_1
    return-void
.end method

.method private static varargs replace(Ljava/util/ArrayList;[Ljava/lang/Object;)V
    .locals 6
    .param p1, "args"    # [Ljava/lang/Object;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/ArrayList",
            "<",
            "Landroid/animation/PropertyValuesHolder;",
            ">;[",
            "Ljava/lang/Object;",
            ")V"
        }
    .end annotation

    .prologue
    .line 165
    .local p0, "props":Ljava/util/ArrayList;, "Ljava/util/ArrayList<Landroid/animation/PropertyValuesHolder;>;"
    const/4 v2, 0x0

    array-length v4, p1

    move v3, v2

    :goto_0
    if-ge v3, v4, :cond_2

    aget-object v0, p1, v3

    .line 166
    .local v0, "killobject":Ljava/lang/Object;
    sget-object v2, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    invoke-virtual {v2, v0}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;

    .line 167
    .local v1, "tween":Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
    if-eqz v1, :cond_0

    .line 168
    iget-object v2, v1, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->animator:Landroid/animation/ObjectAnimator;

    invoke-virtual {v2}, Landroid/animation/ObjectAnimator;->cancel()V

    .line 169
    if-eqz p0, :cond_1

    .line 170
    iget-object v5, v1, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->animator:Landroid/animation/ObjectAnimator;

    .line 171
    invoke-virtual {p0}, Ljava/util/ArrayList;->size()I

    move-result v2

    new-array v2, v2, [Landroid/animation/PropertyValuesHolder;

    invoke-virtual {p0, v2}, Ljava/util/ArrayList;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v2

    check-cast v2, [Landroid/animation/PropertyValuesHolder;

    .line 170
    invoke-virtual {v5, v2}, Landroid/animation/ObjectAnimator;->setValues([Landroid/animation/PropertyValuesHolder;)V

    .line 165
    :cond_0
    :goto_1
    add-int/lit8 v2, v3, 0x1

    move v3, v2

    goto :goto_0

    .line 173
    :cond_1
    sget-object v2, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    invoke-virtual {v2, v1}, Ljava/util/HashMap;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    goto :goto_1

    .line 164
    .end local v0    # "killobject":Ljava/lang/Object;
    .end local v1    # "tween":Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
    :cond_2
    return-void
.end method

.method public static varargs to(Ljava/lang/Object;J[Ljava/lang/Object;)Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
    .locals 17
    .param p0, "object"    # Ljava/lang/Object;
    .param p1, "duration"    # J
    .param p3, "vars"    # [Ljava/lang/Object;

    .prologue
    .line 58
    const-wide/16 v4, 0x0

    .line 59
    .local v4, "delay":J
    const/4 v12, 0x0

    .line 60
    .local v12, "updateListener":Landroid/animation/ValueAnimator$AnimatorUpdateListener;
    const/4 v9, 0x0

    .line 61
    .local v9, "listener":Landroid/animation/Animator$AnimatorListener;
    const/4 v7, 0x0

    .line 64
    .local v7, "interpolator":Landroid/animation/TimeInterpolator;
    new-instance v10, Ljava/util/ArrayList;

    move-object/from16 v0, p3

    array-length v14, v0

    div-int/lit8 v14, v14, 0x2

    invoke-direct {v10, v14}, Ljava/util/ArrayList;-><init>(I)V

    .line 65
    .local v10, "props":Ljava/util/ArrayList;, "Ljava/util/ArrayList<Landroid/animation/PropertyValuesHolder;>;"
    const/4 v6, 0x0

    .end local v7    # "interpolator":Landroid/animation/TimeInterpolator;
    .end local v9    # "listener":Landroid/animation/Animator$AnimatorListener;
    .end local v12    # "updateListener":Landroid/animation/ValueAnimator$AnimatorUpdateListener;
    .local v6, "i":I
    :goto_0
    move-object/from16 v0, p3

    array-length v14, v0

    if-ge v6, v14, :cond_c

    .line 66
    aget-object v14, p3, v6

    instance-of v14, v14, Ljava/lang/String;

    if-nez v14, :cond_0

    .line 67
    new-instance v14, Ljava/lang/IllegalArgumentException;

    new-instance v15, Ljava/lang/StringBuilder;

    invoke-direct {v15}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v16, "Key must be a string: "

    invoke-virtual/range {v15 .. v16}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v15

    aget-object v16, p3, v6

    invoke-virtual/range {v15 .. v16}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v15

    invoke-virtual {v15}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v15

    invoke-direct {v14, v15}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v14

    .line 69
    :cond_0
    aget-object v8, p3, v6

    check-cast v8, Ljava/lang/String;

    .line 70
    .local v8, "key":Ljava/lang/String;
    add-int/lit8 v14, v6, 0x1

    aget-object v13, p3, v14

    .line 72
    .local v13, "value":Ljava/lang/Object;
    const-string/jumbo v14, "simultaneousTween"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-eqz v14, :cond_2

    .line 65
    .end local v13    # "value":Ljava/lang/Object;
    :cond_1
    :goto_1
    add-int/lit8 v6, v6, 0x2

    goto :goto_0

    .line 74
    .restart local v13    # "value":Ljava/lang/Object;
    :cond_2
    const-string/jumbo v14, "ease"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-eqz v14, :cond_3

    move-object v7, v13

    .line 75
    check-cast v7, Landroid/animation/TimeInterpolator;

    .local v7, "interpolator":Landroid/animation/TimeInterpolator;
    goto :goto_1

    .line 76
    .end local v7    # "interpolator":Landroid/animation/TimeInterpolator;
    :cond_3
    const-string/jumbo v14, "onUpdate"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-nez v14, :cond_4

    const-string/jumbo v14, "onUpdateListener"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-eqz v14, :cond_5

    :cond_4
    move-object v12, v13

    .line 77
    check-cast v12, Landroid/animation/ValueAnimator$AnimatorUpdateListener;

    .local v12, "updateListener":Landroid/animation/ValueAnimator$AnimatorUpdateListener;
    goto :goto_1

    .line 78
    .end local v12    # "updateListener":Landroid/animation/ValueAnimator$AnimatorUpdateListener;
    :cond_5
    const-string/jumbo v14, "onComplete"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-nez v14, :cond_6

    const-string/jumbo v14, "onCompleteListener"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-eqz v14, :cond_7

    :cond_6
    move-object v9, v13

    .line 79
    check-cast v9, Landroid/animation/Animator$AnimatorListener;

    .local v9, "listener":Landroid/animation/Animator$AnimatorListener;
    goto :goto_1

    .line 80
    .end local v9    # "listener":Landroid/animation/Animator$AnimatorListener;
    :cond_7
    const-string/jumbo v14, "delay"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-eqz v14, :cond_8

    .line 81
    check-cast v13, Ljava/lang/Number;

    .end local v13    # "value":Ljava/lang/Object;
    invoke-virtual {v13}, Ljava/lang/Number;->longValue()J

    move-result-wide v4

    goto :goto_1

    .line 82
    .restart local v13    # "value":Ljava/lang/Object;
    :cond_8
    const-string/jumbo v14, "syncWith"

    invoke-virtual {v14, v8}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v14

    if-nez v14, :cond_1

    .line 84
    instance-of v14, v13, [F

    if-eqz v14, :cond_9

    .line 85
    const/4 v14, 0x2

    new-array v15, v14, [F

    move-object v14, v13

    .line 86
    check-cast v14, [F

    const/16 v16, 0x0

    aget v14, v14, v16

    const/16 v16, 0x0

    aput v14, v15, v16

    check-cast v13, [F

    .end local v13    # "value":Ljava/lang/Object;
    const/4 v14, 0x1

    aget v14, v13, v14

    const/16 v16, 0x1

    aput v14, v15, v16

    .line 85
    invoke-static {v8, v15}, Landroid/animation/PropertyValuesHolder;->ofFloat(Ljava/lang/String;[F)Landroid/animation/PropertyValuesHolder;

    move-result-object v14

    invoke-virtual {v10, v14}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto :goto_1

    .line 87
    .restart local v13    # "value":Ljava/lang/Object;
    :cond_9
    instance-of v14, v13, [I

    if-eqz v14, :cond_a

    .line 88
    const/4 v14, 0x2

    new-array v15, v14, [I

    move-object v14, v13

    .line 89
    check-cast v14, [I

    const/16 v16, 0x0

    aget v14, v14, v16

    const/16 v16, 0x0

    aput v14, v15, v16

    check-cast v13, [I

    .end local v13    # "value":Ljava/lang/Object;
    const/4 v14, 0x1

    aget v14, v13, v14

    const/16 v16, 0x1

    aput v14, v15, v16

    .line 88
    invoke-static {v8, v15}, Landroid/animation/PropertyValuesHolder;->ofInt(Ljava/lang/String;[I)Landroid/animation/PropertyValuesHolder;

    move-result-object v14

    invoke-virtual {v10, v14}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto/16 :goto_1

    .line 90
    .restart local v13    # "value":Ljava/lang/Object;
    :cond_a
    instance-of v14, v13, Ljava/lang/Number;

    if-eqz v14, :cond_b

    .line 91
    check-cast v13, Ljava/lang/Number;

    .end local v13    # "value":Ljava/lang/Object;
    invoke-virtual {v13}, Ljava/lang/Number;->floatValue()F

    move-result v3

    .line 92
    .local v3, "floatValue":F
    const/4 v14, 0x1

    new-array v14, v14, [F

    const/4 v15, 0x0

    aput v3, v14, v15

    invoke-static {v8, v14}, Landroid/animation/PropertyValuesHolder;->ofFloat(Ljava/lang/String;[F)Landroid/animation/PropertyValuesHolder;

    move-result-object v14

    invoke-virtual {v10, v14}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto/16 :goto_1

    .line 94
    .end local v3    # "floatValue":F
    .restart local v13    # "value":Ljava/lang/Object;
    :cond_b
    new-instance v14, Ljava/lang/IllegalArgumentException;

    .line 95
    new-instance v15, Ljava/lang/StringBuilder;

    invoke-direct {v15}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v16, "Bad argument for key \""

    invoke-virtual/range {v15 .. v16}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v15

    invoke-virtual {v15, v8}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v15

    const-string/jumbo v16, "\" with value "

    invoke-virtual/range {v15 .. v16}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v15

    invoke-virtual {v13}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    move-result-object v16

    invoke-virtual/range {v15 .. v16}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v15

    invoke-virtual {v15}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v15

    .line 94
    invoke-direct {v14, v15}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v14

    .line 100
    .end local v8    # "key":Ljava/lang/String;
    .end local v13    # "value":Ljava/lang/Object;
    :cond_c
    sget-object v14, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    move-object/from16 v0, p0

    invoke-virtual {v14, v0}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v11

    check-cast v11, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;

    .line 101
    .local v11, "tween":Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
    const/4 v2, 0x0

    .line 102
    .local v2, "anim":Landroid/animation/ObjectAnimator;
    if-nez v11, :cond_10

    .line 104
    invoke-virtual {v10}, Ljava/util/ArrayList;->size()I

    move-result v14

    new-array v14, v14, [Landroid/animation/PropertyValuesHolder;

    invoke-virtual {v10, v14}, Ljava/util/ArrayList;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v14

    check-cast v14, [Landroid/animation/PropertyValuesHolder;

    .line 103
    move-object/from16 v0, p0

    invoke-static {v0, v14}, Landroid/animation/ObjectAnimator;->ofPropertyValuesHolder(Ljava/lang/Object;[Landroid/animation/PropertyValuesHolder;)Landroid/animation/ObjectAnimator;

    move-result-object v2

    .line 105
    .local v2, "anim":Landroid/animation/ObjectAnimator;
    new-instance v11, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;

    .end local v11    # "tween":Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
    invoke-direct {v11, v2}, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;-><init>(Landroid/animation/ObjectAnimator;)V

    .line 106
    .restart local v11    # "tween":Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;
    sget-object v14, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    move-object/from16 v0, p0

    invoke-virtual {v14, v0, v11}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    .line 113
    :goto_2
    if-eqz v7, :cond_d

    .line 114
    invoke-virtual {v2, v7}, Landroid/animation/ObjectAnimator;->setInterpolator(Landroid/animation/TimeInterpolator;)V

    .line 118
    :cond_d
    invoke-virtual {v2, v4, v5}, Landroid/animation/ObjectAnimator;->setStartDelay(J)V

    .line 119
    move-wide/from16 v0, p1

    invoke-virtual {v2, v0, v1}, Landroid/animation/ObjectAnimator;->setDuration(J)Landroid/animation/ObjectAnimator;

    .line 120
    if-eqz v12, :cond_e

    .line 121
    invoke-virtual {v2}, Landroid/animation/ObjectAnimator;->removeAllUpdateListeners()V

    .line 122
    invoke-virtual {v2, v12}, Landroid/animation/ObjectAnimator;->addUpdateListener(Landroid/animation/ValueAnimator$AnimatorUpdateListener;)V

    .line 124
    :cond_e
    if-eqz v9, :cond_f

    .line 125
    invoke-virtual {v2}, Landroid/animation/ObjectAnimator;->removeAllListeners()V

    .line 126
    invoke-virtual {v2, v9}, Landroid/animation/ObjectAnimator;->addListener(Landroid/animation/Animator$AnimatorListener;)V

    .line 128
    :cond_f
    sget-object v14, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->mCleanupListener:Landroid/animation/Animator$AnimatorListener;

    invoke-virtual {v2, v14}, Landroid/animation/ObjectAnimator;->addListener(Landroid/animation/Animator$AnimatorListener;)V

    .line 130
    return-object v11

    .line 109
    .local v2, "anim":Landroid/animation/ObjectAnimator;
    :cond_10
    sget-object v14, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->sTweens:Ljava/util/HashMap;

    move-object/from16 v0, p0

    invoke-virtual {v14, v0}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v14

    check-cast v14, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;

    iget-object v2, v14, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->animator:Landroid/animation/ObjectAnimator;

    .line 110
    .local v2, "anim":Landroid/animation/ObjectAnimator;
    const/4 v14, 0x1

    new-array v14, v14, [Ljava/lang/Object;

    const/4 v15, 0x0

    aput-object p0, v14, v15

    invoke-static {v10, v14}, Lcom/mediatek/keyguard/PowerOffAlarm/multiwaveview/Tweener;->replace(Ljava/util/ArrayList;[Ljava/lang/Object;)V

    goto :goto_2
.end method
